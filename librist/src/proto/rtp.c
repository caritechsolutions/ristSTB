...
void rist_rtcp_write_fsr_enable(uint8_t *buf, int *offset, const uint32_t flow_id) {
    struct rist_rtcp_fsr_pkt *fsr = 
        (struct rist_rtcp_fsr_pkt *)(buf + RIST_MAX_PAYLOAD_OFFSET + *offset);
    *offset += sizeof(struct rist_rtcp_fsr_pkt);
    
    // V=2, P=0, Subtype=5 (Enable)
    fsr->rtcp.flags = (2 << 6) | FSR_SUBTYPE_ENABLE;
    fsr->rtcp.ptype = PTYPE_NACK_CUSTOM;  // PT=204
    fsr->rtcp.len = htons(2);             // 3 32-bit words minus 1
    fsr->rtcp.ssrc = htobe32(flow_id);
    memcpy(fsr->name, "RIST", 4);         // 0x52495354
}

void rist_rtcp_write_fsr_disable(uint8_t *buf, int *offset, const uint32_t flow_id) {
    struct rist_rtcp_fsr_pkt *fsr = 
        (struct rist_rtcp_fsr_pkt *)(buf + RIST_MAX_PAYLOAD_OFFSET + *offset);
    *offset += sizeof(struct rist_rtcp_fsr_pkt);
    
    // V=2, P=0, Subtype=6 (Disable)  
    fsr->rtcp.flags = (2 << 6) | FSR_SUBTYPE_DISABLE;
    fsr->rtcp.ptype = PTYPE_NACK_CUSTOM;  // PT=204
    fsr->rtcp.len = htons(2);             // 3 32-bit words minus 1
    fsr->rtcp.ssrc = htobe32(flow_id);
    memcpy(fsr->name, "RIST", 4);         // 0x52495354
